namespace LuaInterface
{
	...
	public class LuaStatic
	{
		...
		public static int loader(IntPtr L)
		{
			...
			LuaDLL.luaL_loadbuffer(L, code, Encoding.UTF8.GetByteCount(code), fileName);
		...
		public static int dofile(IntPtr L)
		{
            ...
            // Load with Unity3D resources
            string code;
			var file = (TextAsset)Resources.Load(fileName);
			if( file == null)
            {
                code = LuaManager.Load(fileName);// Add by Stephen Zhou, to require file as customized
                if (code == null)
                {
			...
			if( LuaDLL.luaL_loadbuffer(L, code, Encoding.UTF8.GetByteCount(code), fileName) == 0 )
			{
